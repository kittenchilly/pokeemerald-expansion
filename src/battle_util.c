...
void HandleAction_UseMove(void)
{
        ...
        && GetBattlerSide(gBattlerAttacker) != GetBattlerSide(gSideTimers[side].followmeTarget))
    {
        gBattleStruct->moveTarget[gBattlerAttacker] = gBattlerTarget = gSideTimers[side].followmeTarget; // follow me moxie fix
           ...
           && (gBattleMoves[gCurrentMove].power != 0 || (moveTarget != MOVE_TARGET_SELF && moveTarget != MOVE_TARGET_ALL))
            ...
            || (GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_STORM_DRAIN && moveType == TYPE_WATER)))
    {
        ...
        if (var == 4)
        {
            if (moveTarget & MOVE_TARGET_RANDOM_NORMAL)
            {
                if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER)
                {
                    if (Random() & 1)
            ...
            else if (moveTarget & MOVE_TARGET_ALL_ADJACENT)
            {
        ...
        else
        {
          ...
          && moveTarget & MOVE_TARGET_RANDOM_NORMAL)
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER)
        {
            if (Random() & 1)
    ...
    else if (moveTarget == MOVE_TARGET_ADJACENT_ALLY)
    {
        if (IsBattlerAlive(BATTLE_PARTNER(gBattlerAttacker)))
          ...
          && moveTarget == MOVE_TARGET_ALL_ADJACENT)
    {
        for (gBattlerTarget = 0; gBattlerTarget < gBattlersCount; gBattlerTarget++)
        {
            if (gBattlerTarget == gBattlerAttacker)
            ...
            if (gBattleMons[gBattlerAttacker].status2 & STATUS2_BIDE)
            {
                ...
                else
                {
                    ...
                    if (gTakenDmg[gBattlerAttacker])
                    {
                        ...
                        if (gAbsentBattlerFlags & gBitTable[gBattlerTarget])
                            gBattlerTarget = GetMoveTarget(MOVE_BIDE, MOVE_TARGET_NORMAL + 1);
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            ...
            if ((gLastUsedAbility == ABILITY_SOUNDPROOF && gBattleMoves[move].soundMove && !(moveTarget & MOVE_TARGET_SELF))
            ...
            {
                if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || !(moveTarget & (MOVE_TARGET_ALL_ADJACENT_FOES | MOVE_TARGET_ALL_ADJACENT)))
                ...
                effect = 1;
                  ...
                  && !(moveTarget & MOVE_TARGET_FOE_SIDE)
    ...
    case ABILITYEFFECT_ABSORBING: // 3
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_WIND_RIDER:
                if (gBattleMoves[gCurrentMove].windMove && !(GetBattlerMoveTargetType(gBattlerAttacker, gCurrentMove) & MOVE_TARGET_SELF))
                    effect = 2, statId = STAT_ATK;
                break;
...
u32 GetMoveTarget(u16 move, u8 setTarget)
{
    ...
    case MOVE_TARGET_RANDOM_NORMAL:
        ...
        if (IsAffectedByFollowMe(gBattlerAttacker, side, move))
            targetBattler = gSideTimers[side].followmeTarget;
        else if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && moveTarget & MOVE_TARGET_RANDOM_NORMAL)
            targetBattler = SetRandomTarget(gBattlerAttacker);
        else
            targetBattler = GetBattlerAtPosition(BATTLE_OPPOSITE(GetBattlerSide(gBattlerAttacker)));
        break;
    ...
    default:
        ...
        break;
    case MOVE_TARGET_ADJACENT_ALLY:
        if (IsBattlerAlive(BATTLE_PARTNER(gBattlerAttacker)))
...
bool32 IsBattlerProtected(u32 battler, u32 move)
{
    ...
    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_UNSEEN_FIST
        ...
        return TRUE;
             ...
             && GetBattlerMoveTargetType(gBattlerAttacker, move) & (MOVE_TARGET_ALL_ADJACENT_FOES | MOVE_TARGET_ALL_ADJACENT))
        return TRUE;
...
bool32 BlocksPrankster(u16 move, u32 battlerPrankster, u32 battlerDef, bool32 checkTarget)
{
    ...
    if (GetBattlerSide(battlerPrankster) == GetBattlerSide(battlerDef))
        return FALSE;
    if (checkTarget && (GetBattlerMoveTargetType(battlerPrankster, move) & (MOVE_TARGET_FOE_SIDE | MOVE_TARGET_SCRIPTED)))
        return FALSE;
